...
func TestClientDevices(t *testing.T) {
	// Fields are present even when their values are zero; presence must be
	// represented separately from the value.
	zeroFields := "private_key=" + strings.Repeat("0", 64) + "\nlisten_port=0\nfwmark=0\npublic_key=" + strings.Repeat("0", 64) + "\npreshared_key=" + strings.Repeat("0", 64) + "\nendpoint=0.0.0.0:0\npersistent_keepalive_interval=0\nerrno=0\n\n"
	zeroDevice, err := parseDevice(strings.NewReader(zeroFields))
	if err != nil {
		t.Fatalf("failed to parse zero fields: %v", err)
	}
	if zeroDevice.PublicKey != zeroDevice.PrivateKey.PublicKey() {
		t.Fatalf("public key was not derived from zero private key: %v", zeroDevice.PublicKey)
	}
	if zeroDevice.HasPublicKey != zeroDevice.HasPrivateKey || !zeroDevice.HasPrivateKey || !zeroDevice.HasListenPort || !zeroDevice.HasFirewallMark {
		t.Fatalf("device field presence not preserved: %+v", zeroDevice)
	}
	if len(zeroDevice.Peers) != 1 || !zeroDevice.Peers[0].HasPresharedKey || !zeroDevice.Peers[0].HasEndpoint || !zeroDevice.Peers[0].HasPersistentKeepaliveInterval {
		t.Fatalf("peer field presence not preserved: %+v", zeroDevice.Peers)
	}
